...
# if timerr>=5:
  try:
    if m.chat.id not in wait_chats.find_one({})['chats']:
      ...
      bot.send_photo(-1001324175427, bot.get_chat(m.chat.id).photo.big_file_id.split('_')[0], caption = 'Найден новый чат: "'+m.chat.title+'" ('+str(m.chat.id)+') ('+str(m.chat.username)+')')
